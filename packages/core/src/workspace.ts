...
export class Workspace {
  ...
  private async runEditorAction(state: EditorState, action: EditorAction, signal?: AbortSignal): Promise<EditorResult> {
    if(action.type==='style-transfer'&&!this.registry.isActive('style-transfer'))throw new CoreError({code:'MODULE_NOT_ACTIVE',message:'样式吸取工具已关闭'});
    ...
    if(action.type==='read-bleed'||action.type==='set-bleed')this.documentBleed=null;
    ...
    let result = await this.scheduler.run('editDocument', async () => {
      ...
      if(action.type==='variable-data'&&action.operation==='capture'&&measured.boardRegions&&measured.ownershipBoards){if(signal?.aborted)throw new Error('操作已取消');return fn.call(this.options.adapter,{...request,action:{...action,artworkAssignments:artboardOwners(measured.boardRegions,measured.ownershipBoards)}});}
    ...
    if(action.type==='read-bleed'||action.type==='set-bleed') { const b=result.bleedOffsets; this.documentBleed=result.status==='completed'&&b?.length===4&&b.every(n=>Number.isFinite(n)&&n>=0)?{session:state.docSessionId,offsets:[...b]}:null; }
