...
function runtime(){
 ...
 const rendererRoot=exe.slice(0,exe.lastIndexOf('/'));
 for(const dependency of ['engines/ghostscript-10.08.0/bin/gswin64c.exe','engines/ghostscript-10.08.0/bin/gsdll64.dll','engines/jpeg-3.2.0/bin/jpegtran.exe','engines/jpeg-3.2.0/bin/jpeg62.dll'])if(e.cep.fs?.stat(rendererRoot+'/'+dependency).err!==0)throw Error('独立大图渲染组件未安装；请配置独立渲染组件。普通尺寸原生导出仍可使用，不会自动降低分辨率。');
